...
/**
 * Pad PCMU with μ-law silence so utterances never hard-start/hard-stop mid
 * frame. 80ms lead-in + 200ms trail-out removes the click/"voice break" at
 * stream boundaries and gives the SBC a clean inter-turn gap.
 */
function padPcmu(buffer) {
  if (!Buffer.isBuffer(buffer) || !buffer.length) return buffer;
  const lead = Buffer.alloc(4 * 160, 0xff);
  const trail = Buffer.alloc(10 * 160, 0xff);
  return Buffer.concat([lead, buffer, trail]);
}

/* Edge Read-Aloud websocket (same free neural voice as the portal). Returns
 * WAV/PCM bytes with no Python and no ffmpeg — primary live-call path. */
...
/** One Edge WS synthesis → PCM WAV buffer (riff 16 kHz) or null. */
...
/**
 * Tier 1 for the phone path: Edge websocket → riff PCM → pure-JS PCMU.
 * No Python, no ffmpeg, no process spawn — keeps media/VAD/heartbeats live
 * and cuts the multi-second inter-turn gap that sounds like a broken line.
 */
async function edgeWsToBuffer(text, { locale, style, rate }) {
  ...
  try {
    ...
    const chunks = splitForWsTts(text);
    // Synthesize sentences in parallel — serial Edge round-trips stacked
    // hundreds of ms onto every turn and sounded like a laggy/broken line.
    const wavs = await Promise.all(chunks.map((chunk) => edgeWsSynth(chunk, voice, ratePct)));
    const parts = [];
    for (const wav of wavs) {
      if (!wav || wav.length < 100) { if (parts.length) break; return null; }
      const mulaw = wavToMulaw(wav);
      if (!mulaw || mulaw.length < 160) { if (parts.length) break; return null; }
      parts.push(mulaw);
    }
    if (!parts.length) return null;
    const buffer = Buffer.concat(parts);
    return buffer.length >= 160 ? { buffer, engine: "edge-ws" } : null;
  } catch {
    return null;
  }
}
...
async function speakToBuffer(text, { locale = "en", style = "human", rate = 1 } = {}) {
  ...
  if (edgeWs) return { ...edgeWs, buffer: padPcmu(edgeWs.buffer) };
  const edge = await edgeToBuffer(text, { locale, style, rate });
  if (edge) return { ...edge, buffer: padPcmu(edge.buffer) };
  const headtts = await headTtsToBuffer(text, { locale, style, rate });
  if (headtts) return { ...headtts, buffer: padPcmu(headtts.buffer) };
  const sapi = await sapiToBuffer(text, { locale, style, rate });
  if (sapi) return { ...sapi, buffer: padPcmu(sapi.buffer) };
  return null;
}

module.exports = { speak, speakEdge, speakHeadTTS, speakWindows, speakToBuffer, edgeVoiceFor, normalizeStyle, styleRate, wavToMulaw, mulawEncode, padPcmu, edgeWsToBuffer };
