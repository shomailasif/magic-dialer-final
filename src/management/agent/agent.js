...
async function runAgent(opts={}){
 ...
 if(opts.call===true){
  const { voiceCall }=require("./call"),{ speakToBuffer }=require("./voice"),{ hearFromBuffer }=require("./hear"),{ createVad }=require("./vad"),{ createLocalRingCentralEngine }=require("./local-ringcentral-engine");
  ...
  const speakFn=async text=>{const r=await speakToBuffer(text,{locale:config.lang||"en",style:config.voiceStyle||"friendly"});if(!r||!Buffer.isBuffer(r.buffer)||r.buffer.length<160)throw new Error("TTS produced no valid PCMU/8000 telephone audio");const n=await engine.sendAudio(r.buffer);log(`[local-media-v2] outbound ${n} bytes PCMU/8000 playback finished`);};
  const listenFn=async()=>{let release;const ended=new Promise(r=>release=r);state={vad:createVad({minSpeechMs:160,endSilenceMs:620}),pre:[],chunks:[],started:false,done:false,resolve:release};const timer=setTimeout(()=>{if(state&&!state.done){state.done=true;state.resolve();}},15000);await ended;clearTimeout(timer);const s=state;state=null;if(!s.started||!s.chunks.length)return null;const audio=Buffer.concat(s.chunks);log(`[local-media-v2] inbound ${audio.length} bytes PCMU/8000`);const text=hearFromBuffer(audio,{sampleRate:8000});log(`[local-media-v2] transcript ${text?"received":"empty"}`);return text;};
