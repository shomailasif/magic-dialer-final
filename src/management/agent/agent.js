...
function takeWatchdogLock(){try{if(fs.existsSync(WATCHDOG_LOCK)){const old=Number(String(fs.readFileSync(WATCHDOG_LOCK,"utf8")).trim());if(old&&old!==process.pid&&pidAlive(old))return false;}fs.mkdirSync(path.dirname(WATCHDOG_LOCK),{recursive:true});fs.writeFileSync(WATCHDOG_LOCK,String(process.pid));return true;}catch{return true;}}
async function runWatchdog(args){if(!takeWatchdogLock())return;const childArgs=args.filter(a=>a!=="--watchdog");childArgs.push("--no-browser");while(true){const child=spawn(process.execPath,childArgs,{stdio:["ignore","inherit","inherit"]});await new Promise(r=>child.on("exit",r));await new Promise(r=>setTimeout(r,4000));}}
...
function bumpStats(c,r){const d=new Date().toISOString().slice(0,10),s=c.stats||{since:d,day:d,calls:0,qualified:0,today:0,qualifiedToday:0,lastScore:0,bestScore:0};if(s.day!==d){s.day=d;s.today=0;s.qualifiedToday=0;}s.calls++;s.today++;if(r.goodLead){s.qualified++;s.qualifiedToday++;}s.lastScore=r.score||0;s.bestScore=Math.max(s.bestScore||0,s.lastScore);c.stats=s;return s;}
...
function takeAgentLock(){if(!isPacked())return true;try{if(fs.existsSync(AGENT_LOCK)){const old=Number(fs.readFileSync(AGENT_LOCK,"utf8"));if(old&&pidAlive(old))return false;}fs.mkdirSync(path.dirname(AGENT_LOCK),{recursive:true});fs.writeFileSync(AGENT_LOCK,String(process.pid));return true;}catch{return true;}}

async function runAgent(opts={}){
 const cfgPath=opts.configPath||defaultConfigPath(),configDir=path.dirname(cfgPath);let config=loadConfig(cfgPath)||{};
 if(!takeAgentLock()){log("Magic Dialer is already running");openDashboardExternal();return;}
 try{localDb.open(configDir);}catch{}
 ...
 if(opts.call===true){
  ...
  let state=null;
  const engine=createLocalRingCentralEngine({number:target,sip:{user:v.username,pass:v.sipPassword,authId:v.extension||v.username,domain:v.server||"sip.ringcentral.com",proxy:v.server||"sip40.ringcentral.com",port:Number(v.port||5096)},onLog:log,onAudio:b=>{if(!state)return;for(let i=0;i<b.length;i+=160){const f=b.subarray(i,i+160),x=state.vad.push(f,20);if(!state.started){state.pre.push(f);if(state.pre.length>10)state.pre.shift();if(x.speaking){state.started=true;state.chunks.push(...state.pre);state.pre=[];}}else{state.chunks.push(f);if(x.ended&&!state.done){state.done=true;state.resolve();}}}}});
  await engine.connect();
  const speakFn=async text=>{const r=await speakToBuffer(text,{locale:config.lang||"en",style:config.voiceStyle||"friendly"});if(!r||!r.buffer)throw new Error("TTS produced no telephone audio");const n=engine.sendAudio(r.buffer);log(`[local-media-v2] outbound ${n} bytes`);};
  const listenFn=async()=>{let release;const ended=new Promise(r=>release=r);state={vad:createVad({minSpeechMs:160,endSilenceMs:620}),pre:[],chunks:[],started:false,done:false,resolve:release};const timer=setTimeout(()=>{if(!state.done){state.done=true;state.resolve();}},15000);await ended;clearTimeout(timer);const s=state;state=null;if(!s.started||!s.chunks.length)return null;const audio=Buffer.concat(s.chunks);log(`[local-media-v2] inbound ${audio.length} bytes`);return await hearFromBuffer(audio,{locale:config.lang||"en",sampleRate:8000});};
  try{const result=await voiceCall({product:config.product,leadFields:config.leadFields||[],persona:config.persona,companyName:config.companyName,callbackNumber:config.callbackNumber,callbackIn:config.callbackIn,contactEmail:config.contactEmail,learning:config.learning,locale:config.lang||"en",voiceStyle:config.voiceStyle||"friendly",speakFn,listenFn,onLog:m=>{log(m);ui({line:m});},onMode:m=>ui({mode:m})});config.learning=result.learning;bumpStats(config,result);pushActivity(config,`Local-media call done - score ${result.score}`);saveConfig(config,cfgPath);}finally{engine.close();}
  if(opts.callOnce)return;
 }

 while(true){try{const res=await post(`${portal}/api/heartbeat`,{token:config.token,voipReady:!!(config.voip&&config.voip.ready),sync:sync.buildSyncPayload()});if(res.status===200&&res.body){if(res.body.disabled){ui({status:"DISABLED",mode:"off",line:"Disabled by admin."});return;}applyPortalConfig(config,res.body.config,cfgPath);if(res.body.sync)sync.processSyncResponse(res.body.sync);ui({status:"ONLINE",mode:"on",line:"heartbeat OK"});}else ui({status:"OFFLINE",mode:"on",line:"Heartbeat rejected"});}catch(e){ui({status:"OFFLINE",mode:"on",line:"Reconnecting to portal..."});}await new Promise(r=>setTimeout(r,HEARTBEAT_INTERVAL_MS));}
}

if(require.main===module){const args=process.argv.slice(2);if(args.includes("--watchdog"))runWatchdog(args);else runAgent({call:args.includes("--call"),callOnce:args.includes("--call-once"),open:!args.includes("--no-browser")}).catch(e=>{console.error(e);process.exitCode=1;});}
