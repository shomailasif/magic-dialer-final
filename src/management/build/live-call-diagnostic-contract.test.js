...
["SIP no prospect text log",!sip.includes('Prospect said:", txt')],["SIP no transcript preview",!sip.includes("transcript?.slice")],
["SIP no spoken text preview",!sip.includes('speak:", text.slice')],["SIP no streaming text preview",!sip.includes('sentences from:", text.slice')],
["SIP no blocked error text preview",!sip.includes("responseText.slice")],["SIP no TTS timeout text preview",!sip.includes("TIMEOUT for:")],
["SIP no tiny-audio text preview",!sip.includes("chunk.slice")],["DNC persistence remains",sip.includes("isSpokenOptOut(txt)")],
["strategy voice remains",sip.includes("voiceForTone(agentConfig.tone)")],["no executable mutation",!sip.includes("eval(")],
["campaign auth unchanged",api.includes("Unauthorized")],["campaign limit remains bounded",api.includes("Math.min(100")],
["diagnostic status 500",api.includes("500,requestId")]];
for(const [n,v] of checks)assert.ok(v,n);console.log("live call diagnostic contract: "+checks.length+"/"+checks.length+" checks PASS");