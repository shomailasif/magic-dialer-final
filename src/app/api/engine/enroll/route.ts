...
export async function POST(req:Request){
 ...
 await prisma.$transaction([
  ...
  prisma.engineDevice.upsert({where:{userId_machineId:{userId:row.userId,machineId}},create:{userId:row.userId,machineId,tokenHash:hash(deviceToken)},update:{tokenHash:hash(deviceToken),revokedAt:null}})
 ]);
